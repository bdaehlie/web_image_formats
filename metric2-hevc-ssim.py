...
def main(argv):
  ...
  while i < len(quality_values):
    percent_improvement += hevc_ssim_equiv_file_sizes[i] / jpeg_file_sizes[i]
    i += 1

  print "Average percentage size of HEVC to JPEG at equiv SSIM: " + str(percent_improvement / len(quality_values))
